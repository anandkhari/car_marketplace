    ...
    >
      <span
        className={`w-1.5 h-1.5 rounded-full shrink-0 transition-colors duration-300 ${
          active
            ? "bg-indigo-600 shadow-sm dark:bg-[#F2F2F7]"
...
function SectionHeader({ label, active, open, onClick }) {
  return (
    ...
    >
      <span>{label}</span>
      <span className="text-xs text-slate-400 dark:text-[#6B6B70]">
        {open ? "∨" : "›"}
      </span>
    </button>
  );
...
function isCurrentSection(dr) {
  return CURRENT_TYPES.includes(dr?.type);
}
function isPreviousSection(dr) {
  return PREVIOUS_TYPES.includes(dr?.type);
}
...
export default function SidePanel({
  ...
  const [customStart, setCustomStart] = useState(() =>
    initCustomStr(dateRange, "start"),
  );
  const [customEnd, setCustomEnd] = useState(() =>
    initCustomStr(dateRange, "end"),
  );
  ...
  const panelContent = (
    <div className="pb-8 mt-10">
      ...
      <div className="flex items-center justify-between px-3 mb-2">
        ...
        <span
          className={`text-sm font-semibold min-w-[48px] text-center ${rawPercentile === 50 ? "text-slate-600 dark:text-[#8E8E93]" : "text-indigo-600 dark:text-[#F2F2F7]"}`}
        >
          P{rawPercentile}
        </span>
        <button
      ...
      <div className="px-3 mb-1">
        <input
          ...
          className="w-full h-1 cursor-pointer appearance-none rounded-full bg-slate-200 dark:bg-[#3A3A3C] bg-gradient-to-r from-indigo-500 to-indigo-500 dark:from-[#F2F2F7] dark:to-[#F2F2F7] bg-no-repeat accent-indigo-500 dark:accent-[#F2F2F7]"
          style={{
            backgroundSize: `${((rawPercentile - 1) / 98) * 100}% 100%`,
          }}
        />
      </div>
      ...
      <div className="flex items-center justify-between px-3 mb-2">
        <button
          onClick={() =>
            onRepeatThresholdChange(Math.max(1, rawRepeatThreshold - 1))
          }
        ...
        <button
          onClick={() =>
            onRepeatThresholdChange(Math.min(5, rawRepeatThreshold + 1))
          }
      ...
      <div className="flex flex-col gap-0.5 pb-2">
        ...
        {openSection === "current" && (
          <div className="flex flex-col gap-0.5 ml-2 mb-1">
            <DateOption
              active={dateRange?.type === "today"}
              onClick={() => onDateChange({ type: "today" })}
            >
              Today
            </DateOption>
            <DateOption
              active={dateRange?.type === "weekToDate"}
              onClick={() => onDateChange({ type: "weekToDate" })}
            >
              Week to date
            </DateOption>
            <DateOption
              active={dateRange?.type === "monthToDate"}
              onClick={() => onDateChange({ type: "monthToDate" })}
            >
              Month to date
            </DateOption>
            <DateOption
              active={dateRange?.type === "quarterToDate"}
              onClick={() => onDateChange({ type: "quarterToDate" })}
            >
              Quarter to date
            </DateOption>
            <DateOption
              active={dateRange?.type === "yearToDate"}
        ...
        {openSection === "previous" && (
          <div className="flex flex-col gap-0.5 ml-2 mb-1">
            <DateOption
              active={dateRange?.type === "yesterday"}
              onClick={() => onDateChange({ type: "yesterday" })}
            >
              Yesterday
            </DateOption>
            <DateOption
              active={dateRange?.type === "lastWeek"}
              onClick={() => onDateChange({ type: "lastWeek" })}
            >
              Last week
            </DateOption>
            <DateOption
              active={dateRange?.type === "lastMonth"}
              onClick={() => onDateChange({ type: "lastMonth" })}
            >
              Last month
            </DateOption>
            <DateOption
              active={dateRange?.type === "lastQuarter"}
              onClick={() => onDateChange({ type: "lastQuarter" })}
            >
              Last quarter
            </DateOption>
            <DateOption
              active={dateRange?.type === "lastYear"}
            ...
            <div className="px-3 py-1">
              <p className="text-[11px] text-slate-400 dark:text-[#6B6B70] mb-1.5">
                Last N
              </p>
              <div className="flex gap-1.5 items-center">
                <input
                  ...
                  value={lastN.n}
                  onChange={(e) =>
                    setLastN({ ...lastN, n: Number(e.target.value) })
                  }
        ...
        {/* Pre-2.0 launch */}
        <DateOption
          active={dateRange?.type === "pre20"}
          onClick={() => onDateChange({ type: "pre20" })}
        >
          Pre-2.0 launch
        </DateOption>
        ...
        {/* Post-2.0 launch */}
        <DateOption
          active={dateRange?.type === "post20"}
          onClick={() => onDateChange({ type: "post20" })}
        >
          Post-2.0 launch
        </DateOption>
        <span className="text-[10px] text-slate-400 dark:text-[#6B6B70] px-6 -mt-1.5 pb-0.5">
        ...
        {openSection === "custom" && (
          <div className="flex flex-col gap-3 px-3 pt-1 pb-2 mx-1">
            <div>
              <p className="text-[11px] text-slate-500 dark:text-[#8E8E93] mb-1.5">
                From
              </p>
              <DateRangePicker
