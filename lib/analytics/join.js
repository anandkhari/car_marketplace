...
function computeGapStats(bookingDates) {
  ...
  return { avgGapDays, medianGapDays, minGapDays, maxGapDays, returnPattern, bookingGaps: gaps }
}
...
const adjustedBookingCount = computeAdjustedBookingCount(
  ...
  AVG_PKG_PRICE,
    ...
    joined.push({
      ...
      bookingGaps:   gapStats.bookingGaps || [],
    })
    matchedCount++
  }
  ...
  // Logic B — pool all individual gaps from every customer group
  const allGaps = []
  for (const g of Object.values(groups)) {
    if (!g.bookingDates || g.bookingDates.length < 2) continue
    const sorted = [...g.bookingDates].sort((a, b) => a - b)
    for (let i = 1; i < sorted.length; i++) {
      const gap = Math.round(
        (sorted[i] - sorted[i - 1]) / (1000 * 60 * 60 * 24)
      )
  ...
  return { joinedCustomers: joined, allGaps }
}
