  ...
  return (
    <div
      ...
      className={`
        ...
        ${!isLoaded && !hasError ? 'border-gray-200 bg-white dark:border-[#2D2D2F] dark:bg-[#242426] hover:border-blue-300 hover:bg-blue-50'  : ''}
      `}
    >
      <input ref={inputRef} type="file" accept=".csv" className="hidden" onChange={handleChange} />

      <p className="text-xs font-semibold text-gray-500 dark:text-[#6B6B70] uppercase tracking-wide mb-3">{label}</p>
      ...
      {!isLoading && !isLoaded && !hasError && (
        <div className="flex items-center gap-2 text-gray-400 dark:text-[#6B6B70]">
...
function Dot({ active }) {
  return (
    <span className={`inline-block w-2 h-2 rounded-full ${active ? 'bg-green-500' : 'bg-gray-300 dark:bg-[#3A3A3C]'}`} />
  )
}
...
function CopyLinkButton({ slug }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="text-xs px-3 py-1 rounded-lg border border-gray-200 dark:border-[#2D2D2F] bg-white dark:bg-[#242426] text-gray-600 dark:text-[#8E8E93] hover:bg-gray-50 dark:hover:bg-[#2D2D2F] transition-colors whitespace-nowrap"
    >
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#1C1C1E]">
      <div className="max-w-2xl mx-auto px-6 py-10">
        ...
        {/* Back link */}
        <div className="mb-6">
          <a href="/" className="text-xs text-gray-400 dark:text-[#6B6B70] hover:text-gray-600 dark:hover:text-[#8E8E93] transition-colors">
            ← Panda Hub
          </a>
        </div>
        ...
        <div className="flex items-center justify-between mb-12">
          <p className="text-xs text-gray-400 dark:text-[#6B6B70]">{loadedCount} of {SLOTS.length} files uploaded</p>
          <button
            ...
            className={`
              ...
              ${allReady && !isSaving
                ...
                : 'bg-gray-100 dark:bg-[#2D2D2F] text-gray-400 dark:text-[#6B6B70] cursor-not-allowed'}
            `}
          >
        ...
        <div>
          <h2 className="text-sm font-semibold text-gray-900 dark:text-[#F2F2F7] mb-1">Published Links</h2>
          <p className="text-xs text-gray-400 dark:text-[#6B6B70] mb-4">Share these links with your team</p>

          {isLoadingFromSupabase ? (
            <div className="bg-white dark:bg-[#242426] border border-gray-100 dark:border-[#2D2D2F] rounded-xl px-5 py-8 flex items-center justify-center gap-3">
              <div className="w-4 h-4 border-2 border-blue-500 border-t-transparent rounded-full animate-spin shrink-0" />
              <p className="text-sm text-gray-500 dark:text-[#8E8E93]">Loading published links...</p>
            </div>
          ) : historyRows.length === 0 ? (
            <div className="bg-white dark:bg-[#242426] border border-gray-100 dark:border-[#2D2D2F] rounded-xl px-5 py-8 text-center">
              <p className="text-sm text-gray-500 dark:text-[#8E8E93] mb-1">No published links yet.</p>
              <p className="text-xs text-gray-400 dark:text-[#6B6B70]">
                Upload data and publish to share with your team.
              </p>
            </div>
          ) : (
            <div className="bg-white dark:bg-[#242426] border border-gray-100 dark:border-[#2D2D2F] rounded-xl overflow-hidden">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-[#2D2D2F] text-gray-400 dark:text-[#6B6B70] uppercase tracking-wide">
                ...
                <tbody>
                  {historyRows.map((row, i) => {
                    ...
                    return (
                      <tr
                        key={row.id}
                        className={`${i < historyRows.length - 1 ? 'border-b border-gray-50 dark:border-[#2D2D2F]' : ''}`}
                      >
                        <td className="px-4 py-3 text-gray-700 dark:text-[#F2F2F7] whitespace-nowrap">
