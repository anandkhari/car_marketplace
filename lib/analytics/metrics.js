...
export function computeReturnIntervals(customers, allGaps = []) {
  const repeaters = customers.filter(
    c => (c.bookingCount ?? 0) >= 2
  )

  if (repeaters.length === 0 || allGaps.length === 0) {
    return {
      avgGapDays:        0,
      medianGapDays:     0,
      fastestReturn:     0,
  ...
  // Pure Method B — all individual gaps pooled across all customers
  ...
  const fastestReturn = sortedGaps[0]
  const slowestReturn = sortedGaps[sortedGaps.length - 1]

  // Distribution by customer return pattern
